...
def sleepy(n_simulations, num_generations=10000, N=50, m=1, b=1, gc=20, mu=0, r=5e-7, L=5000, mu_selection_rates="5e-8", 
         ...
         mutation_in_seeds=True, stop_after_mrca=False, debug_print=False, output_name="run", percent_fixed=1, generations_post_fixation_threshold=1,add_mutations_after_fixation=True,
         output_directory="./",n_parallel=10, continue_from=0, print_cmd=False, slurm=False,slurm_start=0, slurm_divide=2, slurm_limit=200
        ):
    
    kwargs = {"num_generations":num_generations,"N":N, "m":m, "b":b, "gc":gc, "mu":mu, "r":r, "L":L, "mu_selection_rates":mu_selection_rates, 
         "selection_coefficients":selection_coefficients,"dominance_coefficients":dominance_coefficients, "selection_positions":selection_positions, "selection_activation_generation":selection_activation_generation,"percent_fixed":percent_fixed,
         "mutation_in_seeds":mutation_in_seeds, "stop_after_mrca":stop_after_mrca, "debug_print":debug_print, "output_name":output_name, 
         "output_directory":output_directory,"generations_post_fixation_threshold": generations_post_fixation_threshold, "add_mutations_after_fixation":add_mutations_after_fixation }
